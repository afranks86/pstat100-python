...
<img src="figures/example-table.png" style="width:1000px">

(There's a reaason that tables are usually put in appendices.)
...
Geometric objects are the things depicted on a plot, whatever those may be.

Typically, they are points, lines, paths, shapes, and areas. 
...
labels = alt.Chart(points).mark_line().encode(
    x = alt.X('z', title = 'Age'),
    y = alt.Y('y', title = 'Greatness of socks as birthday gift')
).properties(
    width = 300,
    height = 300
)
...
In the plot:
* population $\longrightarrow$ y-coordinate;
* year $\longrightarrow$ x-coordinate;
...
One can see spread, center, skewness, and outliers, but _**also shape**_. 
...
<img src="figures/example-graphic.png" style ="width:500px">
