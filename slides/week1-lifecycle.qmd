...
In intro stat, you learn about methods of inference for the following set-up:

* start with a question
* identify a study population
* draw a random (or probability) sample
* collect data

. . .

In practice, one commonly works with found data or convenient data, effectively inverting this picture:

* start with a topic
* obtain conveniently available data
* assess relevant population, if any
* determine potential for inference, prediction, etc.
* articulate answerable questions
...
Focusing on the nonmissing values, we see the same power law relationship but with *different* proportionality constants for (at least two of) the three classes of animals.
