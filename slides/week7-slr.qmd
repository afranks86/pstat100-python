...
And compute:
$$
\hat{y}_{new} = \hat{\beta}_0 + \hat{\beta}_1\log(86000)
$$

. . .

In `statsmodels`, there is a `.predict` method to perform this calculation:
```{python}
#| code-fold: false
# prediction
...
There are *two* kinds of variability about a prediction:

1. variability of the mean (due to estimation error)
2. variability of the observation (due to estimation error and random error)

. . .

The first is smaller, because it's easier to estimate the average value for a given $x$ than the exact value:
...
$$
\text{var}(\widehat{\mathbb{E}y_{new}}) = \sigma^2\left(\mathbf{x}_{new}'(\mathbf{X'X})^{-1}\mathbf{x}_{new}\right)
$$

. . .

So the estimated standard deviation $\mathbf{x}_{new}$ is:
$$
SE\left(\widehat{\mathbb{E}y_{new}}\right) = \sqrt{\hat{\sigma}^2\left(\mathbf{x}_{new}'(\mathbf{X'X})^{-1}\mathbf{x}_{new}\right)}
$$

. . .

For derivations of these facts, see PSTAT126.

## Uncertainty about the mean

The value of the standard error is computed as an attribute of the results of `.get_prediction()`:
```{python}
#| code-fold: false
pred_rslt = slr.fit().get_prediction(newobs) 
pred_rslt.se_mean
```

. . .

But a 95% confidence interval can also be returned (mean $\pm$ 2SE)
```{python}
#| code-fold: false
pred_rslt.conf_int(obs = False)
```
...
## Uncertainty about predicted observations
...
Note that this is equal to the estimated variance of the mean at $\mathbf{x}_{new}$ plus the residual variance -- it will be relatiely large whenever there is a lot of unexplained variability.
...
Think of the difference as being between two types of predictions:
...
The uncertainty associated with predicting observations is nearly 20x larger than the uncertainty associated with predicting the mean value:
```{python}
#| code-fold: false
pred_rslt.se_obs/pred_rslt.se_mean
```

. . .

This is attributable to the model capturing only 14% of variability in the raw data.

## Prediction uncertainty

The prediction uncertainty is considerable, but consistent with the variability we see in the data.

![](figures/fig8-prediction.png)

## Summary

This was our first week on statistical models.

* A statistical model is at its core a probability distribution representing a data-generating process.

* The distributions you know and love from PSTAT120A are all simple models.

. . .

Our focus was on the **simple linear regression model**, according to which _**one variable of interest is a linear function of another variable and a random error**_.
