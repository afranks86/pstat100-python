...
y_train = train.gap
slr_part = sm.OLS(endog = y_train, exog = x_train)
```

. . .

Note that the coefficient estimates are a little different:

```{python}
print('fit to full data: ', slr.fit().params)
print('fit to training partition: ', slr_part.fit().params.values)
```

As are the error variance estimates:

```{python}
print('fit to full data: ', slr.fit().scale)
print('fit to training partition: ', slr_part.fit().scale)
```
...
## MSE on training partition

Compare with MSE computed using fitted values:

```{python}
# note, these are just model residuals
fit_errors = train.gap - slr_part.fit().fittedvalues

# training rmse
print('rmse on training partition: ',np.sqrt((fit_errors**2).mean()))
print('rmse on test partition: ', rmse)
```

. . .

Note that 'training MSE' -- MSE computed using the fitted values -- is simply the estimate of the error variance, rescaled by $\frac{n - 1}{n}$.
