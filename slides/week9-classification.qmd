...
Instead of manually examining lots of confusion matrices, it is easier to plot sensitivity against specificity across all unique classification thresholds.
...
* the closer the curve gets to the upper left side, the better the classifier
...
In ML-speak, we've "trained a classifier to predict diabetes".

. . .

To assess predictive accuracy, use data partitioning and compute accuracy metrics:
...
An alternative to logistic regression is probit regression, where:
...
* similar assumptions -- independence and monotonicity
...
Centering the data helps a bit -- then the change in estimated probability associated with a 1-unit change from the mean is:

$$
\Phi\left(\hat{\beta}_j\right) - \underbrace{0.5}_{\Phi(0)}
$$
...
norm.cdf(fit_probit_ctr.params) - 0.5
```